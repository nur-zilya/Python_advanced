...
def sum2():
    ...
    for encoded_char in request_data.split("&"):
        k,v = encoded_char.split("=")

        arrays[k] = [int(it) for it in v.split(",")]


    res_str = ",".join(str(a1 + a2) for (a1, a2) in zip(arrays["array1"], arrays["array2"]))

    return f"Your result is {res_str}"
