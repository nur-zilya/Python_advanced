...
async def show_receipts() -> list[schemas.BaseReceipt]:
    db_receipts = await session.execute(
        select(models.Receipts).order_by(
            models.Receipts.views.desc(), models.Receipts.time_cook
        )
    )
    return [
        schemas.BaseReceipt.from_orm(db_receipt) for db_receipt in db_receipts.scalars()
    ]


@app.get("/receipts/{receipt_id}", response_model=schemas.BaseReceipt)
async def view_receipt(receipt_id: int):
    db_receipt = await session.execute(
        select(models.Receipts).filter(models.Receipts.id == receipt_id)
    )
